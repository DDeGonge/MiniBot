...
void onDataRecv(const uint8_t *mac_addr, const uint8_t *incomingData, int len) {
  ...
  if (len == sizeof(MagneticFieldResponse)) {
    ...
    if (magField.msg_type == MSG_TYPE_MAG_REQUEST) {
      ...
      DEBUG_PRINTF("  Field: [%.2f, %.2f, %.2f] gauss\n",
                   magField.field_x_gauss, magField.field_y_gauss,
                   magField.field_z_gauss);
  ...
  DEBUG_PRINTF("Warning: Unhandled message type or size mismatch (len=%d, "
               "AckMsg=%d, NackMsg=%d, MagField=%d)\n",
               len, sizeof(AckMessage), sizeof(NackMessage),
  ...
  while (1) {
    ...
    if (xQueueReceive(commandQueue, &msg, pdMS_TO_TICKS(10)) == pdPASS) {
      if (msg.commandType == CMD_TYPE_MOT_TEST) {
        ...
        if (cmd->requestType == 1) {
          ...
          if (result == ESP_OK) {
            ...
            if (cmd->targetID == 0xFF) {
              ...
              while ((millis() - startTime) < 500) {
                if (xQueueReceive(ackQueue, &ack, pdMS_TO_TICKS(10)) ==
                    pdPASS) {
                  if (ack.msg_type == MSG_TYPE_ACK_MESSAGE) {
                    DEBUG_PRINTF(
                        "ACK received from robot 0x%02X (broadcast req)\n",
                        ack.responderID);
                    GUIStatus status;
                    status.targetID = ack.responderID;
                    status.ackReceived = true;
              ...
              while ((millis() - startTime) < 500) {
                if (xQueueReceive(ackQueue, &ack, pdMS_TO_TICKS(10)) ==
                    pdPASS) {
                  if (ack.responderID == cmd->targetID &&
          ...
          if (result == ESP_OK) {
            ...
            if (cmd->targetID == 0xFF) {
              // Broadcast request: collect all responses during the full window
              while ((millis() - startTime) < 500) {
                if (xQueueReceive(magFieldQueue, &magResp, pdMS_TO_TICKS(10)) ==
                    pdPASS) {
                  if (magResp.msg_type == MSG_TYPE_MAG_FIELD_RESPONSE) {
                    DEBUG_PRINTF("Magnetic field response from robot 0x%02X "
                                 "(broadcast req)\n",
                                 magResp.responderID);
                    GUIStatus status;
              ...
              while ((millis() - startTime) < 500) {
                if (xQueueReceive(magFieldQueue, &magResp, pdMS_TO_TICKS(10)) ==
                    pdPASS) {
                  if (magResp.responderID == cmd->targetID &&
                      magResp.msg_type == MSG_TYPE_MAG_FIELD_RESPONSE) {
                    respReceived = true;
                    DEBUG_PRINTF(
                        "Magnetic field response received from robot 0x%02X\n",
                        cmd->targetID);
                    GUIStatus status;
                    status.targetID = cmd->targetID;
                    status.ackReceived = true;
              ...
              if (!respReceived) {
                DEBUG_PRINTF(
                    "No magnetic field response from robot 0x%02X (timeout)\n",
                    cmd->targetID);
                GUIStatus status;
                status.targetID = cmd->targetID;
                status.ackReceived = false;
          ...
          DEBUG_PRINTF("Sending position command to robot 0x%02X: (%.2f, %.2f) "
                       "%.2frad %.2fms\n",
                       cmd->targetID, cmd->x, cmd->y, cmd->angle,
                       cmd->duration);
        ...
        syncCmd.timeout_ms =
            (POS_SYNC_INITIAL_DELAY_MS +
             (POS_SYNC_BURST_COUNT + 1) * POS_SYNC_BURST_INTERVAL_MS +
             EMAG_FRAME_LEN_MS);
        ...
        for (int i = 0; i < POS_SYNC_BURST_COUNT; i++) {
          ...
          result = esp_now_send(broadcastAddress, (uint8_t *)&syncPulse,
                                sizeof(syncPulse));
          if (result != ESP_OK) {
